...
from db_setup import Base, MatchV1, MatchV2
import util
import uuid
...
def migrate_from_matchv1_to_matchv2():
	...
	for match in session.query(MatchV1).all():
		session.add(MatchV2(
...
def add_matchv2(match_id: str, event_name: str, team_number: int, match_number: int, auto_line_cross: bool,
	...
	session = DBSession()
	stored_match = session.query(MatchV2).filter(MatchV2.match_id == match_id).first()
	if stored_match is not None:
		stored_match = stored_match  # type: MatchV2
		...
		if stored_match_last_modified_date > new_match_last_modified_date:
			...
			session.query(MatchV2).filter(MatchV2.match_id == match_id).delete()
	session.add(MatchV2(
...
def events_recorded():
	session = DBSession()
	events = [event_name for event_name in session.query(MatchV2.event_name).distinct()]
	session.close()
	return events
...
def matrix_data_for_event(event_name):
	...
	for match in session.query(MatchV2).filter(MatchV2.event_name == event_name).all():  # type: MatchV2
