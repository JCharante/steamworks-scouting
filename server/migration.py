...
def migrate_matchv4_to_matchv5():
	...
	for match in session.query(MatchV4).all():  # type: MatchV4
		existing_entry = session.query(MatchV5).filter(MatchV5.match_id == match.match_id).first()
		if existing_entry is None:
			...
			session.query(MatchV4).filter(MatchV4.match_id == match.match_id).delete()
			session.commit()
	session.close()


def migrate_matchv5_to_matchv6():
	session = DBSession()
	for match in session.query(MatchV5).all():  # type: MatchV5
		existing_entry = session.query(MatchV6).filter(MatchV6.match_id == match.match_id).first()
		if existing_entry is None:
			session.add(MatchV6(
				match_id=match.match_id,
				event_name=match.event_name,
			...
			session.query(MatchV5).filter(MatchV5.match_id == match.match_id).delete()
			session.commit()
	session.close()
